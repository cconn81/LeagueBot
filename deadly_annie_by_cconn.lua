...
local Annie        = { projSpeed = 1.0, aaParticles = {"AnnieBasicAttack_tar", "AnnieBasicAttack_tar_frost", "AnnieBasicAttack2_mis", "AnnieBasicAttack3_mis"}, aaSpellName = {"anniebasicattack"}, startAttackSpeed = "0.579",  }
local MinionInfo = { }
...
----------[[Script Config Menu]]
CfgControls, menu = uiconfig.add_menu('1. Annie Controls', 200)
...
CfgSettings, menu = uiconfig.add_menu('2. Annie Settings', 200)
menu.checkbutton('Save_Stun_ONOFF', 'Save Stun', true)
...
menu.slider('QRNG', 'Q Range', 100, 625, 600, nil, true)
menu.slider('WRNG', 'W Range', 100, 625, 600, nil, true)
menu.slider('ERNG', 'E Range', 100, 625, 600, nil, true)
...
menu.slider('Auto_Harass_Value', 'Auto Harass Value', 0, 100, 50, nil, true)
	...
	menu.permashow('Save_Stun_ONOFF')
	
CfgMasteries, menu = uiconfig.add_menu('3. Annie Masteries', 200)
...
function SaveStun()
	if CfgSettings.Save_Stun_ONOFF then
		if StunCheck() then
			Should_Stun = false
		end
	else
...
function Farm()
	...
	for i, Minion in pairs(Minions) do
		if Minion ~= nil then
			...
			local aaTime = Ping + aaDelay + ( GetDistance2D(myHero, Minion) / Annie.projSpeed )
...
function OnProcessSpell(unit, spell)
	...
	if unit.charName == myHero.charName then
		for i, aaSpellName in pairs(Annie.aaSpellName) do
...
function Combo()  -- Spells in order of priority - Q, W, E when spell is off cooldown. R / Ignite / Exhaust if toggled. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if GetDistance(target) <= 600 then
			if CfgControls.ComboR and myHero.SpellLevelR > 0.0 and myHero.SpellTimeR > 1.0 and StunCheck() and GetDistance(myHero, target) <= CfgSettings.RRNG then
				R()
...
function Harass() -- Spells in order of priority - Q, W, E when spell is off cooldown. No ult / summoner spells. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if GetDistance(target) <= 600 then
			if myHero.SpellTimeW > 1.0 and myHero.SpellLevelW > 0.0 and GetDistance(myHero, target) <= CfgSettings.WRNG then
				W()
			elseif myHero.SpellTimeQ > 1.0 and  myHero.SpellLevelQ > 0.0 and GetDistance(myHero, target) <= CfgSettings.QRNG then
				Q()
			elseif myHero.SpellTimeE > 1.0 and myHero.SpellLevelE > 0.0 and GetDistance(myHero, target) <= CfgSettings.ERNG then
...
function Q() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast Q on target
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.QRNG and myHero.SpellTimeQ > 1.0 and myHero.mana >= (55 + (myHero.SpellLevelQ * 5)) then
				CastSpellTarget('Q',target)
		end
	end
end
...
function W() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast W on target
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.WRNG and myHero.SpellTimeW > 1.0 and myHero.mana >= (60 + (myHero.SpellLevelW * 10)) then
			CastSpellTarget('W',target)
		end
	end
end

function E() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast E on Annie
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.ERNG and myHero.SpellTimeE > 1.0 and myHero.mana >= 20 then
			CastSpellTarget('E',myHero)
		end
	end
end

function R() --If R spell lvl is 1 or higher and target is within configured range and spell is off cooldown and enough mana to cast then it will cast R on GETMEC location or Target
...
function Auto_Harass() --will execute Harass combo until Annie's mana drops below the configured value while target is within spell range
    ...
    for i = 1, objManager:GetMaxHeroes()  do
    	...
    	if (enemy ~= nil and enemy.team ~= myHero.team and enemy.visible == 1 and enemy.invulnerable==0 and enemy.dead == 0) then
			...
			if CfgSettings.Combo_Circles_ONOFF then
				if myHero.SummonerD == 'SummonerDot' and myHero.SpellTimeD > 1.0 or myHero.SummonerF == 'SummonerDot' and myHero.SpellTimeF > 1.0 then
					if qdmg+wdmg+ignitedmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
				end
				if myHero.SpellTimeR > 1.0 then
					if myHero.SummonerD == 'SummonerDot' and myHero.SpellTimeD > 1.0 or myHero.SummonerF == 'SummonerDot' and myHero.SpellTimeF > 1.0 then
						if rdmg+wdmg+qdmg+ignitedmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
					end
				end
				if myHero.SpellTimeR > 1.0 then
					if rdmg+wdmg+qdmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
				end
			end
        	if CfgSettings.Auto_Kill_Steal_ONOFF then
        		if qdmg > enemy.health and myHero.SpellTimeQ > 1.0 and GetDistance(myHero,enemy) <= 625 then
        		    CastSpellTarget("Q",enemy)
				end
				if wdmg > enemy.health and myHero.SpellTimeW > 1.0 and GetDistance(myHero,enemy) <= 625 then
...
----------[[IGERs Auto Level]]
local skillingOrder = {
    Annie = {W,Q,Q,W,Q,R,Q,W,E,Q,R,W,W,E,E,R,E,E},
}
    ...
    if CfgSettings.AutoLevelSpells_ONOFF and IsLolActive() and IsChatOpen() == 0 then
        ...
        if attempts <= 10 or (attempts > 10 and GetTickCount() > lastAttempt+1500) then
            if spellLevelSum < myHero.selflevel then
                ...
                letter = skillingOrder["Annie"][spellLevelSum+1]
