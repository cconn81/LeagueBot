...
function spellFarm(range, func1, func2, func3, func4)
	...
	if minionTarget and myHero.mana >= myHero.maxMana * (menu.ManaManager.manaSpellFarm / 100) then
		...
		if yayo.Config.LaneClear and func1 ~= x then
			if menu._LaneClear.farmQ and getDmg('Q', minionTarget, myHero) >= minionTarget.health then func1(minionTarget) end
		end
		if yayo.Config.LaneClear and func2 ~= x then
			if menu._LaneClear.farmW and getDmg('W', minionTarget, myHero) >= minionTarget.health then func2(minionTarget) end
		end
		if yayo.Config.LaneClear and func3 ~= x then
			if menu._LaneClear.farmE and getDmg('E', minionTarget, myHero) >= minionTarget.health then func3(minionTarget) end
		end
		if yayo.Config.LaneClear and func4 ~= x then
			if menu._LaneClear.farmR and getDmg('R', minionTarget, myHero) >= minionTarget.health then func4(minionTarget) end
		end
	end
end
