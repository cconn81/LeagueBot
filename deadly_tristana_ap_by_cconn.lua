--[[Deadly AP Tristana V1.1 BETA by CCONN based on Deadly Cassiopeia v1.6]]
require 'Utils'
...
local eRNG = (650 + (myHero.SpellLevelE * 9))
----------[[End of Deadly AP Tristana Variables]]
...
local Tristana = { projSpeed = 2.25, aaParticles = {"TristannaBasicAttack_mis"}, aaSpellName = {"tristanabasicattack"}, startAttackSpeed = "0.656",}
local MinionInfo = { }
...
----------[[Script Config Menu]]
CfgControls, menu = uiconfig.add_menu('1. AP Trist Controls', 200)
...
CfgSettings, menu = uiconfig.add_menu('2. AP Trist Settings', 200)
menu.checkbutton('AutoCarry_ONOFF', 'Enable Auto Carry', true)
menu.checkbutton('ComboQ', 'Use Q in Combo', false)
...
menu.slider('WRNG', 'W Range', 100, 900, 900, nil, true)
menu.slider('RRNG', 'R Range', 100, 645, 645, nil, true)
menu.slider('Auto_Harass_Value', 'Auto Harass Value', 0, 100, 50, nil, true)
	menu.permashow('Auto_Harass_ONOFF')
	
CfgMasteries, menu = uiconfig.add_menu('3. AP Trist Masteries', 200)
...
function Draw()
	...
	if myHero.SpellLevelE > 0.00 and myHero.SpellTimeE > 1.0 then CustomCircle(eRNG,2,4,myHero) end
...
function Farm()
	...
	for i, Minion in pairs(Minions) do
		if Minion ~= nil then
			...
			local aaTime = Ping + aaDelay + ( GetDistance2D(myHero, Minion) / Tristana.projSpeed )
...
function OnProcessSpell(unit, spell)
	...
	if unit.charName == myHero.charName then
		for i, aaSpellName in pairs(Tristana.aaSpellName) do
...
----------[[Combo Functions]]
function Combo()  -- Spells in order of priority - W, E when spell is off cooldown. R / Ignite / Exhaust if toggled. Orbwalks after spells are cast. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if GetDistance(target) <= 900 then
			if myHero.SpellTimeW > 1.0 and GetDistance(myHero, target) <= CfgSettings.WRNG then
				W()
			elseif myHero.SpellTimeE > 1.0 and GetDistance(myHero, target) <= eRNG then
				E()
			...
			if CfgSettings.ComboQ and myHero.SpellTimeQ > 1.0 and GetDistance(myHero, target) <= range then
				Q()
			end
			if CfgSettings.AutoCarry_ONOFF and myHero.SpellTimeE < 1.0 and myHero.SpellTimeW < 1.0 and GetDistance(myHero, target) <= range then
				AutoCarry()
			end
		end
		if CfgSettings.MoveToMouse then MoveToMouse() end
...
function Harass() -- Casts E only on target for harass and then orbwalks
	if target ~= nil then
		if GetDistance(target) <= eRNG then
			if myHero.SpellTimeE > 1.0 and GetDistance(myHero, target) <= eRNG then
				E()
			end
			if CfgSettings.AutoCarry_ONOFF and myHero.SpellTimeE < 1.0 and GetDistance(myHero, target) <= range then
				AutoCarry()
			end
		end
		if CfgSettings.MoveToMouse then MoveToMouse() end
...
----------[[Spell Functions]]
function Q() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast Q on Tristana
	if target ~= nil then
		if GetDistance(myHero, target) <= range and myHero.SpellTimeQ > 1.0 and myHero.mana >= 50 then
...
function W() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast W on target
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.WRNG and myHero.SpellTimeW > 1.0 and myHero.mana >= 80 then
...
function E() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast E on target
	if target ~= nil then
		if GetDistance(myHero, target) <= eRNG and myHero.SpellTimeE > 1.0 and myHero.mana >= (40 + (myHero.SpellLevelE * 10)) then
			CastSpellTarget('E',target)
		end
	end
end

function R() --If R spell lvl is 1 or higher and target is within configured range and spell is off cooldown and enough mana to cast then it will cast R on target
	if target ~= nil and myHero.SpellLevelR > 0.0 then
		if GetDistance(myHero, target) <= CfgSettings.RRNG and myHero.SpellTimeR > 1.0 and myHero.mana >= 100 then
...
function KillSteal()
    for i = 1, objManager:GetMaxHeroes()  do
    	...
    	if (enemy ~= nil and enemy.team ~= myHero.team and enemy.visible == 1 and enemy.invulnerable==0 and enemy.dead == 0) then
			...
			if CfgSettings.Combo_Circles_ONOFF then  --Combo Circle functions are here
				if myHero.SummonerD == 'SummonerDot' and myHero.SpellTimeD > 1.0 or myHero.SummonerF == 'SummonerDot' and myHero.SpellTimeF > 1.0 then
					if wdmg + edmg + ignitedmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
				end
				if myHero.SpellLevelR > 0.0 and myHero.SpellTimeR > 1.0 then
					if myHero.SummonerD == 'SummonerDot' and myHero.SpellTimeD > 1.0 or myHero.SummonerF == 'SummonerDot' and myHero.SpellTimeF > 1.0 then
						if wdmg+edmg+rdmg+ignitedmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
					end
				end
				if myHero.SpellLevelR > 0.0 and myHero.SpellTimeR > 1.0 then
					if wdmg + edmg + rdmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
				end
			end
        	if CfgSettings.Auto_Kill_Steal_ONOFF then  --Kill Steal functions are here
				if wdmg > enemy.health and myHero.SpellTimeW > 1.0 and GetDistance(myHero,enemy) <= 900 then
        		    CastSpellXYZ("W",GetFireahead(enemy,2,20))
				end
				if edmg > enemy.health and myHero.SpellTimeE > 1.0 and GetDistance(myHero,enemy) <= range + 100 then
...
----------[[IGERs Auto Level]]
local skillingOrder = {
    Tristana = {E,W,E,W,E,R,E,W,E,W,R,W,Q,Q,Q,R,Q,Q},
}
    ...
    if CfgSettings.AutoLevelSpells_ONOFF and IsLolActive() and IsChatOpen() == 0 then
        ...
        if attempts <= 10 or (attempts > 10 and GetTickCount() > lastAttempt+1500) then
            if spellLevelSum < myHero.selflevel then
                ...
                letter = skillingOrder["Tristana"][spellLevelSum+1]
...
OnLoad()
----------[[Auto Carry]]
SetTimerCallback("DeadlyAPTristana")