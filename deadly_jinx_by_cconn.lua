...
local Jinx = { projSpeed = 2.4, aaParticles = {"Jinx_Q_Minigun_Mis", "Jinx_Q_Rocket_mis"}, aaSpellName = {"JinxBasicAttack", "JinxBasicAttack2", "JinxBasicAttack3", "JinxBasicAttack4", "JinxBasicAttack5", "JinxBasicAttack6", "JinxQAttack", "JinxQAttack2", "JinxCritAttack", "JinxCritAttack2", "JinxCritAttack3"}, startAttackSpeed = "0.625" }
local MinionInfo = { }
...
----------[[Script Config Menu]]
CfgControls, menu = uiconfig.add_menu('1. Jinx Controls', 200)
...
CfgSettings, menu = uiconfig.add_menu('2. Jinx Settings', 200)
...
menu.slider('WRNG', 'W Range', 100, 1500, 1500, nil, true)
menu.slider('ERNG', 'E Range', 100, 900, 900, nil, true)
menu.slider('RRNG', 'R Range', 100, 18100, 2500, nil, true)
menu.slider('Auto_Harass_Value', 'Auto Harass Value', 0, 100, 50, nil, true)
	menu.permashow('Auto_Harass_ONOFF')
	
CfgMasteries, menu = uiconfig.add_menu('3. Jinx Masteries', 200)
...
menu.checkbutton('BilgewaterCutless_ONOFF', 'Bilgewater Cutless', true)
menu.checkbutton('BladeoftheRuinedKing_ONOFF', 'Blade of the Ruined King', true)
menu.checkbutton('ExecutionerCalling_ONOFF', 'Executioners Calling', true)
menu.checkbutton('Smart_BOTRK_ONOFF', 'SMART Blade of the Ruined King', false)
...
function DeadlyJinx()
	if IsLolActive() and IsChatOpen() == 0 then
		target = GetWeakEnemy('PHYSICAL', 1500)
		targetHero = GetWeakEnemy('MAGIC', 1500)
		targetSMDR = GetWeakEnemy('PHYSICAL', 11000)
		Mastery_Damage()
		Auto_Switcharoo()
...
function Auto_Switcharoo()
	local fishboneRange = range + (50 + (myHero.SpellLevelQ * 25))
	if myHero.range == 525 then
		Stance = powpow
		DrawText("Pow Pow",10,40,0xFF00EE00);
	end    
	if myHero.range > 525 then
		Stance = fishbones
		DrawText("FishBones",10,40,0xFF00EE00);
	end
	if target ~= nil then
...
function CCONN_Items()
	local target = GetWeakEnemy('PHYS',range)
	if target ~= nil then
		...
		if CfgItems.BilgewaterCutless_ONOFF then
			if GetDistance(myHero,target) <= 450 then
				useBilgewaterCutless()
			end
		end
		if CfgItems.BladeoftheRuinedKing_ONOFF then
			if CfgItems.Smart_BOTRK_ONOFF then
				if GetDistance(myHero,target) <= 450 then
					if myHero.maxHealth - myHero.health <= target.maxHealth*(15 / 100) then
						useBladeoftheRuinedKing()
					end
				end
			elseif GetDistance(myHero,target) <= 450 then
				useBladeoftheRuinedKing()
			end
		end
		if CfgItems.ExecutionerCalling_ONOFF then
			if GetDistance(myHero,target) <= range then
				useExecutionerCalling()
			end
		end
	end
end
...
function useBladeoftheRuinedKing()
	if target ~= nil then
...
function useExecutionerCalling()
	if target ~= nil then
		GetInventorySlot(3123)
		UseItemOnTarget(3123,myHero)
	end
end

function useMercurialScimitar() --Link to cleanse function
	GetInventorySlot(3139)
	UseItemOnTarget(3139,myHero)
end

function useMuramana()
	GetInventorySlot(3042)
	UseItemOnTarget(3042,myHero)
end

function useOdynsVeil()
	GetInventorySlot(3180)
	UseItemOnTarget(3180,myHero)
end

function useRavenousHydra()
	GetInventorySlot(3174)
	UseItemOnTarget(3174,myHero)
end

function useSwordoftheDivine()
	GetInventorySlot(3131)
	UseItemOnTarget(3131,myHero)
end

function useYoumuuGhostBlade()
	GetInventorySlot(3142)
	UseItemOnTarget(3142,myHero)
end

function useShurelyaRevelrie()
	GetInventorySlot(3069)
	UseItemOnTarget(3069,myHero)
end

function useRanduinOmen()
	GetInventorySlot(3143)
	UseItemOnTarget(3143,myHero)
end

function useQuicksilverSash()
	GetInventorySlot(3140)
	UseItemOnTarget(3140,myHero)
end

function useMikaelCrucible()
	GetInventorySlot(3222)
	UseItemOnTarget(3222,myHero)
end

function useLocketoftheIronSolari()
	GetInventorySlot(3190)
	UseItemOnTarget(3190,target)
end

function useEntropy()
	GetInventorySlot(3184)
	UseItemOnTarget(3184,target)
end

function useBannerofCommand() --Need custom target function for minion
	GetInventorySlot(3060)
	UseItemOnTarget(3060,target)
end
...
function Farm()
	...
	for i, Minion in pairs(Minions) do
		if Minion ~= nil then
			...
			local aaTime = Ping + aaDelay + ( GetDistance2D(myHero, Minion) / Jinx.projSpeed )
...
function OnProcessSpell(unit, spell)
	...
	if unit.charName == myHero.charName then
		for i, aaSpellName in pairs(Jinx.aaSpellName) do
...
function LaneClear() --
    ...
    if target ~= nil and target.visible == 1 and target.dead == 0 then 
		...
		--[[elseif CfgSettings.LC_Q_ONOFF and myHero.SpellTimeQ > 1.0 and getDmg('Q',target,myHero)+(getDmg('Q',target,myHero)*(HavocDamage + ExecutionerDamage)) >= target.health then
			Q()
		elseif CfgSettings.LC_W_ONOFF and myHero.SpellTimeW > 1.0 and getDmg('W',target,myHero)+(getDmg('W',target,myHero)*(HavocDamage + ExecutionerDamage)) >= target.health then
			W()
...
----------[[Combo Functions]]
function Combo()  -- Spells in order of priority - Q, W, E when spell is off cooldown. R / Ignite / Exhaust if toggled. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if myHero.SpellTimeW > 1.0 and GetDistance(myHero, target) <= CfgSettings.WRNG then
			W()
		elseif myHero.SpellTimeE > 1.0 and GetDistance(myHero, target) <= CfgSettings.ERNG then
			E()
		elseif CfgControls.ComboR and myHero.SpellTimeR > 1.0 and GetDistance(myHero, target) <= CfgSettings.RRNG then
			R()
		end
		if CfgSettings.AutoCarry_ONOFF then AutoCarry() end
...
function Harass() -- Spells in order of priority - Q, W, E when spell is off cooldown. No ult / summoner spells. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if myHero.SpellTimeW > 1.0 and GetDistance(myHero, target) <= CfgSettings.WRNG then
			W()
		elseif myHero.SpellTimeE > 1.0 and GetDistance(myHero, target) <= CfgSettings.ERNG then
...
function W()
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.WRNG and myHero.SpellTimeW > 1.0 and myHero.mana >= (35 + (myHero.SpellLevelW * 10)) then
			CastSpellXYZ('W',GetFireahead(target,6.45,24))
		end
	end
end
...
function E()
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.ERNG and myHero.SpellTimeE > 1.0 and myHero.mana >= (26 - (myHero.SpellLevelE * 2)) then
...
function R()
	if target ~= nil and myHero.SpellLevelR > 0.0 then
		if GetDistance(myHero, target) <= CfgSettings.RRNG and myHero.SpellTimeR > 1.0 and myHero.mana >= (105 - (myHero.SpellLevelR * 15)) then
...
--[[function Auto_Harass()
	if targetHero ~= nil then
		--set toggle modes for auto harass - W, or W,Q-AA (rocket)
...
function KillSteal()
    for i = 1, objManager:GetMaxHeroes()  do
    	...
    	if (enemy ~= nil and enemy.team ~= myHero.team and enemy.visible == 1 and enemy.invulnerable==0 and enemy.dead == 0) then
    		local qdmg = getDmg("Q",enemy,myHero) + (getDmg("Q",enemy,myHero)*(HavocDamage + ExecutionerDamage))
			local wdmg = getDmg("W",enemy,myHero) + (getDmg("W",enemy,myHero)*(HavocDamage + ExecutionerDamage))
    		local edmg = getDmg("E",enemy,myHero) + (getDmg("E",enemy,myHero)*(HavocDamage + ExecutionerDamage))
			local rdmg = getDmg("R",enemy,myHero) + (getDmg("R",enemy,myHero)*(HavocDamage + ExecutionerDamage))
			local ignitedmg = (myHero.selflevel*20)+50
			if CfgSettings.Combo_Circles_ONOFF then
				if myHero.SummonerD == 'SummonerDot' and myHero.SpellTimeD > 1.0 or myHero.SummonerF == 'SummonerDot' and myHero.SpellTimeF > 1.0 then
					if qdmg+wdmg+edmg+qdmg+ignitedmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
				end
				if myHero.SpellTimeR > 1.0 then
					if myHero.SummonerD == 'SummonerDot' and myHero.SpellTimeD > 1.0 or myHero.SummonerF == 'SummonerDot' and myHero.SpellTimeF > 1.0 then
						if qdmg+wdmg+edmg+qdmg+rdmg+qdmg+ignitedmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
					end
				end
				if myHero.SpellTimeR > 1.0 then
					if qdmg+wdmg+edmg+qdmg > enemy.health then CustomCircle(50,10,5,enemy) DrawTextObject("ComboKill!", enemy, Color.Red) end
				end
			end
        	if CfgSettings.Auto_Kill_Steal_ONOFF then
        		if qdmg > enemy.health and myHero.SpellTimeQ > 1.0 and GetDistance(myHero,enemy) <= 600 then
        		    CastSpellTarget("Q",enemy)
				end
				if wdmg > enemy.health and myHero.SpellTimeW > 1.0 and GetDistance(myHero,enemy) <= 600 then
    ...
    if CfgSettings.AutoLevelSpells_ONOFF and IsLolActive() and IsChatOpen() == 0 then
        ...
        if attempts <= 10 or (attempts > 10 and GetTickCount() > lastAttempt+1500) then
            if spellLevelSum < myHero.selflevel then
                ...
                letter = skillingOrder["Jinx"][spellLevelSum+1]
...
function GetAAData()
    return {  
        Jinx = { projSpeed = 2.4, aaParticles = {"Jinx_Q_Minigun_Mis", "Jinx_Q_Rocket_mis"}, aaSpellName = {"JinxBasicAttack", "JinxBasicAttack2", "JinxBasicAttack3", "JinxBasicAttack4", "JinxBasicAttack5", "JinxBasicAttack6", "JinxQAttack", "JinxQAttack2", "JinxCritAttack", "JinxCritAttack2", "JinxCritAttack3"}, startAttackSpeed = "0.625" },
    }
end
...
SetTimerCallback("DeadlyJinx")
--SetTimerCallback("Auto_Switcharoo")