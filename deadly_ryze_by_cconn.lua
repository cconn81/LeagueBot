--[[	
 _______   _______     ___       _______   __      ____    ____   .______     ____    ____  ________   _______ 
|       \ |   ____|   /   \     |       \ |  |     \   \  /   /   |   _  \    \   \  /   / |       /  |   ____|
|  .--.  ||  |__     /  ^  \    |  .--.  ||  |      \   \/   /    |  |_)  |    \   \/   /  `---/  /   |  |__   
|  |  |  ||   __|   /  /_\  \   |  |  |  ||  |       \_    _/     |      /      \_    _/      /  /    |   __|  
|  '--'  ||  |____ /  _____  \  |  '--'  ||  `----.    |  |       |  |\  \----.   |  |       /  /----.|  |____ 
|_______/ |_______/__/     \__\ |_______/ |_______|    |__|       | _| `._____|   |__|      /________||_______|
.______   ____    ____     ______   ______   ______   .__   __. .__   __.                                      
|   _  \  \   \  /   /    /      | /      | /  __  \  |  \ |  | |  \ |  |                                      
|  |_)  |  \   \/   /    |  ,----'|  ,----'|  |  |  | |   \|  | |   \|  |                                      
|   _  <    \_    _/     |  |     |  |     |  |  |  | |  . `  | |  . `  |                                      
|  |_)  |     |  |       |  `----.|  `----.|  `--'  | |  |\   | |  |\   |                                      
|______/____  |__|_      _\______| \______| \______/  |__| \__| |__| \__|                                      
\   \  /   /    /_ |    | || |                                                                                 
 \   \/   /      | |    | || |_                                                                                
  \      /       | |    |__   _|                                                                               
   \    / __     | |  __   | |                                                                                 
...
----------[[Script Config Menu]]
CfgControls, menu = uiconfig.add_menu('1. Ryze Controls', 200)
...
menu.keydown('SnareCombo', 'Snare Combo', Keys.A)
...
menu.checkbutton('Smart_SnareCombo_ONOFF', 'Smart Snare Combo', true)
...
menu.slider('RRNG', 'R Range', 100, 600, 600, nil, true)
menu.slider('Auto_Harass_Value', 'Auto Harass Value', 0, 100, 50, nil, true)
	menu.permashow('Auto_Harass_ONOFF')
	
CfgMasteries, menu = uiconfig.add_menu('3. Ryze Masteries', 200)
...
function DeadlyRyze()
	if IsLolActive() and IsChatOpen() == 0 then
		...
		if CfgControls.SnareCombo then SnareCombo() end
		if CfgControls.Combo then
			if GetDistance(myHero,target) > 500 and GetTargetDirection(target) == FLEEING then
				if CfgSettings.Smart_SnareCombo_ONOFF then
					SnareCombo()
				end
			else
				Combo()
		...
		for i, Enemy in pairs(Enemies) do
			if Enemy ~= nil then
				...
				if myHero.selflevel >= 6 and myHero.SpellTimeR > 1.0 then
					Current_Burst = Round(QDMG + WDMG + EDMG + QDMG + RDMG + QDMG, 0) --Show damage of QWEQRQ combo if Ult is available
				else
					Current_Burst = Round(QDMG + WDMG + EDMG + QDMG, 0) --Show damage of QWEQ combo if Ult is not available
				end
...
function Farm()
	...
	for i, Minion in pairs(Minions) do
		if Minion ~= nil then
			...
			local aaTime = Ping + aaDelay + ( GetDistance2D(myHero, Minion) / Ryze.projSpeed )
...
function OnProcessSpell(unit, spell)
	...
	if unit.charName == myHero.charName then
		for i, aaSpellName in pairs(Ryze.aaSpellName) do
...
function Combo()  -- Spells in order of priority - Q, W, E when spell is off cooldown. R / Ignite / Exhaust if toggled. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if GetDistance(target) <= 600 then
			if myHero.SpellTimeQ > 1.0 and GetDistance(myHero, target) <= CfgSettings.QRNG then
				Q()
			elseif myHero.SpellTimeW > 1.0 and GetDistance(myHero, target) <= CfgSettings.WRNG then
				W()
...
function SnareCombo()  -- Spells in order of priority - W, Q, E when spell is off cooldown. R / Ignite / Exhaust if toggled. Move to mouse between spells and when there's no target.
	if target ~= nil then
		if GetDistance(target) <= 600 then
			if myHero.SpellTimeW > 1.0 and GetDistance(myHero, target) <= CfgSettings.WRNG then
				W()
			elseif myHero.SpellTimeQ > 1.0 and GetDistance(myHero, target) <= CfgSettings.QRNG then
				Q()
			elseif myHero.SpellTimeE > 1.0 and GetDistance(myHero, target) <= CfgSettings.ERNG then
				E()
			elseif CfgControls.ComboR and myHero.SpellTimeR > 1.0 and GetDistance(myHero, target) <= CfgSettings.RRNG then
...
----------[[Spell Functions]]
function Q() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast Q on target
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.QRNG and myHero.SpellTimeQ > 1.0 and myHero.mana >= 60 then
				CastSpellTarget('Q',target)
		end
	end
end

function W() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast W on target
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.WRNG and myHero.SpellTimeW > 1.0 and myHero.mana >= (50 + (myHero.SpellLevelW * 10)) then
			CastSpellTarget('W',target)
		end
	end
end

function E() --If target is within configured range and spell is off cooldown and enough mana to cast then it will cast E on target
	if target ~= nil then
		if GetDistance(myHero, target) <= CfgSettings.ERNG and myHero.SpellTimeE > 1.0 and myHero.mana >= (80 + (myHero.SpellLevelE * 10)) then
			CastSpellTarget('E',target)
		end
	end
end

function R() --If R spell lvl is 1 or higher and target is within configured range and spell is off cooldown and enough mana to cast then it will cast R on Ryze
	if target ~= nil and myHero.SpellLevelR > 0.0 then
		if GetDistance(myHero, target) <= CfgSettings.RRNG and myHero.SpellTimeR > 1.0 and myHero.mana >= (90 - (myHero.SpellLevelR * 20)) then
			CastSpellTarget('R',myHero)
		end
	end
end

function Auto_Harass() --will execute Harass combo until Ryze's mana drops below the configured value while target is within spell range
    ...
    if CfgSettings.AutoLevelSpells_ONOFF and IsLolActive() and IsChatOpen() == 0 then
        ...
        if attempts <= 10 or (attempts > 10 and GetTickCount() > lastAttempt+1500) then
            if spellLevelSum < myHero.selflevel then
                ...
                letter = skillingOrder["Ryze"][spellLevelSum+1]
